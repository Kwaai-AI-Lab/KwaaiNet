...
`DEBIAN/md5sums` is generated by `build-deb.sh` (dpkg-deb does not write it),
so `dpkg -V kwaainet` and `debsums` actually verify the payload.
...
`p2pd` is not packaged. The Go libp2p daemon was removed upstream (#191) —
nodes run rust-libp2p in process. `stage-payload.sh` copies only `kwaainet`,
so an older archive that still carries `p2pd` is *ignored* rather than
rejected, and `test-install.sh` asserts the package ships no `p2pd`, so it
cannot come back by accident.

## The packaged marker

`/usr/lib/kwaainet/packaged` contains `deb`. The Rust updater reads it (and
requires the binary to be `/usr/bin/kwaainet`) and refuses to self-update,
because a self-update would overwrite files dpkg owns and leave the package
database disagreeing with the filesystem. `kwaainet update` still checks for
a newer release and prints the `curl … && sudo dpkg -i …` command that
installs it; `kwaainet uninstall` leaves the binary to `apt remove`.
...
`build-deb.sh --lintian` runs `lintian --fail-on error`, so an error tag fails
the build; warnings and info print and pass.
...
The version comes from `GITHUB_REF_NAME` (passed into the build container by
`package-linux`; it has no `git`) or, locally, `git describe --exact-match`,
falling back to `[workspace.package] version` in `core/Cargo.toml`, because
`release.yml`'s `plan.outputs.tag` is `github.ref_name` — a *branch* under
`workflow_dispatch`. Only a tag that looks like a version (`v1.2.3` or `1.2.3`)
is accepted; a branch name falls through to `Cargo.toml`. A tag that disagrees
with `Cargo.toml` fails the build rather than attaching a mis-versioned `.deb`
to the release. `KWAAINET_VERSION` overrides all of it.
