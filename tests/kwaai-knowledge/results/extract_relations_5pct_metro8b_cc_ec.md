...
**CC quote:** `one of my sister’s friends`
...
**Extracted relations:**
- `My mother` **parent_of** `Ayesha Rassool`
...
**CC quote:** `one of Gandhi’s sons fell in love with one of Goulam Gool’s sisters`
...
**CC pass (raw):**
```json
{"quote": "Alie Fataar, former secretary of the TLSA."}
```

**CC quote:** `Alie Fataar, former secretary of the TLSA.`

**EC pass (raw):**
```json
Since there is no family relationship mentioned in the quote, the output will be an empty array.
...
**CC pass (raw):**
```json
{"quote": "my aunt Jane Gool arrived"}
```

**CC quote:** `my aunt Jane Gool arrived`
...
**Extracted relations:**
- `Yousuf Rassool` **parent_of** `Bibi Gool`
...
**CC quote:** `the entire Abed family were originally from Durban and had apparently settled in the Cape ages ago without the required permission.`
