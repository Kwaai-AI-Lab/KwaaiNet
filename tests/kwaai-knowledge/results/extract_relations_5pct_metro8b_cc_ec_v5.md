...
**EC pass (raw):**
```json
There is no family relationship stated in the quote. The quote only mentions that certain individuals were served with banning orders, but it does not mention any family relationships between them.
...
**Extracted relations:**
- `Yousuf Rassool` **sibling_of** `Jane Gool-Tabata`
