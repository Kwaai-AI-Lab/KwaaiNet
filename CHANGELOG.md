...
- `is_routable_v6` / `is_globally_routable_v6` mirror the IPv4 classifiers. The transition ranges (Teredo, 6to4, ORCHID) are never announceable; unique-local `fc00::/7` and the documentation and benchmarking prefixes follow `require_global_ips`, as RFC2544 and RFC5737 do on the IPv4 side. IPv4-mapped addresses are classified as the IPv4 address they carry.
- A `require_global_ips` config key (default `true`), wired through to the daemon's address policy and to AutoNAT's `only_global_ips`. Set `false` only on a network deliberately built on IANA-reserved space.
...
### Changed
- **IANA-reserved address space is rejected by default.** RFC2544 `198.18/15`, the RFC5737 documentation ranges and IPv6 unique-local were previously announceable on every node because the internal `require_global_ips` switch defaulted off and was not reachable from config. A deployment that relies on such addresses must now set `require_global_ips: false`.
