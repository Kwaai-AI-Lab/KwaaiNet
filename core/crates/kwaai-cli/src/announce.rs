...
//!
//! 1. The **record value types** — [`VpkInfo`], [`DHTServerInfo`], [`ModelInfo`]
//!    and [`dht_id`] — moved verbatim out of `node.rs`. Their msgpack encodings
//!    are a published wire format: `map.kwaai.ai`'s crawler and Python Hivemind
//!    both decode them, so these must not be "improved" without a coordinated
//!    change on the consumer side. `node.rs` re-exports them and is otherwise
//!    unchanged.
//!
//! 2. The **native builders and sender** — [`build_announce_records`],
//!    [`build_unannounce_records`] and [`send_records_via_handle`] — which
//!    produce exactly today's [`StoreRequest`]s and push them through a
//!    [`NetworkHandle`] instead of a p2pd control socket.
//!
//! The p2pd path in `node.rs` keeps working untouched; run_node is rewired in
//! Phase 4. Unit tests below assert the native builders are byte-for-byte
//! identical to what `node.rs::announce` sends today.
//!
...
/// because the builders are called from two places (announce and re-announce)
/// and a transposed `total_blocks`/`start_block` pair would be a silent wire
...
/// Build every STORE request a single announcement round sends, in the order
/// `node.rs::announce` sends them: blocks, `_petals.models`,
/// `_kwaai.inference.nodes`, then `_kwaai.vpk.nodes` when VPK is enabled.
///
/// The block range comes from `server_info.start_block..server_info.end_block`,
/// matching today's behaviour where the caller has already reconciled the
/// configured range with what the shard actually serves.
///
/// All expirations are `now + `[`ANNOUNCE_TTL_SECS`]. They are computed once per
/// call so every record in one round shares a timestamp — the same property
/// today's code gets from calling `get_dht_time()` repeatedly within a few
/// microseconds, made explicit.
...
///
/// Only the block records and the VPK record are rewritten — matching
/// `node.rs::unannounce`, which leaves `_petals.models` alone (it describes the
/// model, not this server) and does not currently touch
...
/// treats as "remove immediately". The record then ages out naturally, exactly
/// as a missed re-announce would.
...
///
/// The shape `node.rs` feeds into `reputation::ReputationStore` — the announce
/// path doubles as the reputation probe, so no extra RPCs are needed.
...
/// (`store_ok` containing a `true`) — the same bar `node.rs::send_to_bootstrap`
/// applies, and the right one: a partially reachable bootstrap set is normal.
///
/// # Differences from the p2pd path
///
/// * **No manual timeout.** `node.rs` wraps each call in
///   `tokio::time::timeout(30s, …)` because the p2pd client has no per-call
///   budget. [`NetworkHandle::call_unary_handler`] applies
///   `NetworkConfig::request_timeout` itself and reports an expiry as
///   [`kwaai_p2p::P2PError::Timeout`], so the native node sets that config to
///   30 s and this function just awaits.
/// * **Dial on demand.** The handle dials the bootstrap if not already
///   connected, matching Go's `host.NewStream`; the p2pd path deliberately
///   avoided `connect_peer` because that crashed p2pd's goroutines.
...
mod tests {
    ...
    #[test]
    fn record_values_are_the_moved_encoders_verbatim() {
        let p = peer();
