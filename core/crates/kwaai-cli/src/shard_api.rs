...
use crate::display::*;
use crate::hf;
use crate::shard_cmd::{discover_chain, forward_through_chain, sample_token, BlockServerEntry, daemon_socket};
...
async fn run_inference(
    ...
    let mut token_ids: Vec<u32> = match state.tokenizer.encode(&prompt) {
        Ok(ids) => ids,
        Err(e)  => { let _ = tx.send(format!("[tokenizer error: {e}]")).await; return; }
    };
    ...
    loop {
        ...
        let last_logits = if logits_shape.len() == 3 && logits_shape[1] > 1 {
            ...
            match logits_tensor.i((0, seq_len - 1, ..)) {
                Ok(t)  => t,
                Err(e) => { let _ = tx.send(format!("[slice error: {e}]")).await; break; }
            }
        } else {
            match logits_tensor.flatten_all() {
                Ok(t)  => t,
                Err(e) => { let _ = tx.send(format!("[flatten error: {e}]")).await; break; }
            }
        };

        let next_id = match sample_token(&last_logits, temperature, top_k, top_p) {
            Ok(id) => id as u32,
            Err(e) => { let _ = tx.send(format!("[sample error: {e}]")).await; break; }
        };
...
fn make_chat_sse(rx: tokio::sync::mpsc::Receiver<String>, model_id: String) -> Response {
    let ctx = SseCtx { rx, id: make_id("chatcmpl"), model_id, created: unix_now() };
...
fn make_completion_sse(rx: tokio::sync::mpsc::Receiver<String>, model_id: String) -> Response {
    let ctx = SseCtx { rx, id: make_id("cmpl"), model_id, created: unix_now() };
...
async fn collect_chat(
    ...
    Json(ChatCompletionResponse {
        ...
        usage: Usage { prompt_tokens: 0, completion_tokens: n, total_tokens: n },
    })
    .into_response()
}
...
pub async fn run(args: ShardApiArgs) -> Result<()> {
    ...
    print_success(&format!("API server ready — http://localhost:{}/v1", args.port));
    println!("  Model:  {}", model_ref);
    println!();
    println!("  Try:");
    println!("    curl http://localhost:{}/v1/models", args.port);
    println!("    curl http://localhost:{}/v1/chat/completions \\", args.port);
