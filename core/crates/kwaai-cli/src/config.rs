...
pub struct ContributeConfig {
    ...
    ///
    /// It is an `Option` because flipping a plain `bool` default cannot
    /// distinguish "never chosen" from "explicitly set",
...
/// Every read of config.yaml parses through here. It drops a leading UTF-8
/// BOM: PowerShell 5.1 and Notepad write one, and serde_yaml reports it as
/// "more than one document".
fn from_yaml<T: serde::de::DeserializeOwned>(text: &str) -> serde_yaml::Result<T> {
...
impl KwaaiNetConfig {
    ...
    ///
    /// Lives beside the flag it reads, so every announce path agrees on what
    /// ONLINE means.
    ...
    pub fn peek_log_level() -> Option<String> {
        ...
        from_yaml::<LogLevelOnly>(&text).ok()?.log_level
    }
    ...
    pub fn load_or_create() -> Result<Self> {
        ...
        if cfg_file.exists() {
            ...
            let mut cfg: KwaaiNetConfig =
                from_yaml(&text).with_context(|| format!("parsing {}", cfg_file.display()))?;
    ...
    pub fn set_key(&mut self, key: &str, value: &str) -> Result<()> {
        match key {
            ...
            "native_p2p" => {
                anyhow::bail!("native_p2p is no longer configurable — the node always runs the in-process p2p stack")
            }
...
#[cfg(test)]
mod native_p2p_tombstone {
    use super::*;

    #[test]
    fn a_config_carrying_the_dead_key_still_parses() {
        let cfg: KwaaiNetConfig =
...
mod tests {
    ...
    fn config_with_utf8_bom_parses() {
        ...
        let cfg: KwaaiNetConfig = from_yaml(text).expect("the loader strips the BOM");
