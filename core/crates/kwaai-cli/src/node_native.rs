...
//!   to change its announce addresses — deferring the restart while RPC streams
//!   are in flight — and caches relay addresses on disk to paper over AutoRelay's
//!   ~7-minute latency. Natively there is nothing to restart and nothing to
//!   cache: reachability changes arrive on a watch channel and the record is
...
/// announcing twice. This is the *second* of two independent guards — the watch
/// channel is already equality-gated in the service, so address churn never
...
/// | periodic IDENTIFY + restart | **replaced** — a reachability change arrives on a watch channel and re-announces in place |
...
pub async fn run_native_node(
    ...
    // `using_relay` now comes from the swarm rather than from configuration: it
    // is true when a circuit reservation is actually confirmed. The p2pd path
