...
///
/// This previously wrote a `PersistentConnectionResponse`, whose
/// `callUnaryResponse` sits at field **2**. Decoded as a
/// `PersistentConnectionRequest`, field 2 is `AddUnaryHandlerRequest`, a proto2
/// message with `required string proto` and `required bool balanced` — neither
/// present — so every Go caller failed to unmarshal the reply. Every inbound
/// rpc_ping/store/find a Rust node served to a Go-daemon caller errored out.
...
mod tests {
    ...
    /// The oneof arm must land on field 4. Field 2 is the historical bug.
    #[test]
    ...
    /// Regression proof that the old shape was genuinely broken.
    ///
    /// `PersistentConnectionResponse{callId, callUnaryResponse}` puts its oneof
    /// arm at field 2. A caller decoding a `PersistentConnectionRequest` — which
    /// is what `Daemon.exchangeMessages` does — finds `AddUnaryHandlerRequest`
    /// there instead. Go's gogo (proto2) rejects the message outright for the
    /// absent `required bool balanced`. prost is laxer and *accepts* it, but
    /// mis-decodes: the DHT response payload is reinterpreted as a handler
    /// registration name. `GetUnaryResponse()` is nil either way, so no caller
    /// can recover the response bytes.
    #[test]
    fn old_response_shape_does_not_yield_a_unary_response() {
