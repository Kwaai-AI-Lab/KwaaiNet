...
//!
//! Folding a "raw mode" flag into `unary::Handler` would have meant two
//! unrelated lifetime policies behind one set of queues, and every unary
//! invariant — per-call timeout, `max_concurrent_streams`, callId correlation —
//! would have needed an "unless raw" caveat. The unary path is untouched by
//! this module.
//!
//! ## Shape
//!
