...
//!
//! Phase 4 slice 2 adds the NAT-traversal set:
//!
...
//!
//! The struct is `#[derive(NetworkBehaviour)]` so adding a field is additive —
//! the generated `KwaaiBehaviourEvent` gains a variant and the service's event
//! loop gets a new match arm. That match is exhaustive, so the two land in one
//! commit or nothing compiles.
