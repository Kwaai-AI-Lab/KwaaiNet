...
impl NetworkService {
    ...
    fn handle_command(&mut self, command: Command) {
        match command {
            ...
            // No pending-map entry: once dispatched, the behaviour owns
            // `reply` and resolves it on every outcome, dial failures and
            // timeouts included. (While parked in `pending_routed`, the
            // service owns it — see `fail_all_pending`.)
            Command::CallUnary {
                peer,
                proto,
                data,
