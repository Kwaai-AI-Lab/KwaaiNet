...
fn bench_trigger_paths() {
    ...
    let y = std::fs::read_to_string(
        "/Users/rezarassool/Source/KwaaiNet/tests/kwaai-knowledge/ontologies/D6.yaml").unwrap();
    ...
    for s in &sentences { let tr = o.triggers(); kwaai_rag::graph::best_ontology_trigger(s, &tr); }
    let naive = t.elapsed();

    let t = std::time::Instant::now();
    let tr = o.triggers();
    for s in &sentences { kwaai_rag::graph::best_ontology_trigger(s, &tr); }
    ...
    let build = t.elapsed();
    let t = std::time::Instant::now();
    for s in &sentences { idx.find_trigger(s); }
    ...
    let t = std::time::Instant::now();
    for c in &cands { o.classify_candidate(c); }
    let cls_naive = t.elapsed();
    let t = std::time::Instant::now();
    for c in &cands { idx.classify(c); }
    let cls_idx = t.elapsed();

    println!("  trigger  naive/sentence-rebuild : {naive:?}");
    println!("  trigger  hoisted Vec            : {hoisted:?}");
    println!("  trigger  OntologyIndex          : {indexed:?}  ({:.1}x vs hoisted, {:.0}x vs naive)",
             hoisted.as_secs_f64()/indexed.as_secs_f64(), naive.as_secs_f64()/indexed.as_secs_f64());
    println!("  index build (once per run)      : {build:?}");
    println!("  classify naive                  : {cls_naive:?}");
    println!("  classify OntologyIndex          : {cls_idx:?}  ({:.1}x)",
