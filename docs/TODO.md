...
- [ ] **Release build link time** — `[profile.release]` in `core/Cargo.toml` uses `lto = true` (fat LTO) + `codegen-units = 1`. Switch to `lto = "thin"`: thin LTO is parallel (5–10× faster link) with <5% runtime impact for this workload (I/O-bound: network, Ollama calls, SQLite). (The "~1.5 hours" originally recorded here is stale — a from-scratch release build of `-p kwaainet` measured ~5 minutes on 2026-08-31.) The `[profile.ci]` profile is already fast (`lto = false`, `codegen-units = 16`) and is unaffected.
...
The map moved to its own repo: https://github.com/Kwaai-AI-Lab/KwaaiNetMap

v1 — the Python service that serves `/api/v1/state` — is on `main` there. v2,
the Rust API and React SPA that used to be `core/crates/map-server` and
`apps/map` here, is parked on the `map-v2` branch along with the backlog that
used to fill this section.
