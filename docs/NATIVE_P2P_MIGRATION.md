...
- **Behaviour additions**: `autonat` (client+server; `only_global_ips` from new
  `only_global_ips` config, default true; boot_delay 5s, retry 30s, refresh 5min,
...
- **`addresses.rs` classifier**: port of node.rs `is_announceable_addr` /
  ...
  `198.18.0.20` as routable. Superseded: `only_global_ips` (default on) rejects them;
  `false` restores the lenient tier.
