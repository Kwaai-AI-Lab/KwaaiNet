...
| `only_global_ips` | `true` | `true` | Reject IANA-reserved space (RFC2544, RFC5737, IPv6 unique-local and the v6 documentation prefixes) the way private ranges are rejected. Leave it on; a bootstrap serving a sealed network built on reserved space is the only reason to set `false`. |
